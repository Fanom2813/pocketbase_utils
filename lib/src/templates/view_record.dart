...
String viewRecordClassGenerator(int lineLength) {
  ...
  final classCode = code_builder.Class(
    (c) => c
      ...
      ..fields.addAll([
        for (var field in viewFields) field.toCodeBuilder(className),
      ])
      ..constructors.addAll([
        code_builder.Constructor((d) => d
          ..constant = true
          ..optionalParameters.addAll([
            for (var field in viewFields)
      ...
      ..methods.addAll([
        code_builder.Method((m) => m
          ...
          ..body = code_builder.literalList([
            for (var field in viewFields) code_builder.refer(field.nameInCamelCase),
          ]).code),
      ]),
  );
